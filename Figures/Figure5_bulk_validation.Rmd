---
title: "Figure5_bulk_validation.Rmd"
...
Bulk transcriptomics data as well as survival data used as validation cohorts in this study are publicly available under the links below, with the following exceptions: 
- Survival data from Löffler-Wirth et al. 2019 was kindly provided by Hans Binder and team (please contact our corresponding authors for access)
- Survival data from Lenz et al. 2008 and Reddy et al. 2017 was kindly provided by Georg Lenz and team (please contact our corresponding authors for access)
...
- Load scRNA-seq Seurat object presented in this study (Zenodo)
...
The signature matrix used downstream for cell type deconvolution is available in Zenodo ("signature_matrix.txt", see link in the manuscript).
```{r}
# Signature matrix from scRNA-seq experiments described in Fig. 1A-B

filtered_signature_matrix <- read.table("/path/signature_matrix.txt", header = TRUE, sep = "\t")
...
## Load data
```{r}
loeffler <- readRDS("/path/loeffler_wirth_nonBL_symbolsconv.rds")
```
...
```{r}
table(metadata$Type_binned2)
```

## Chemokine boxplots (Fig. 2C)
```{r}
pdf(paste0(out_dir, "loeffler_box_org_final.pdf"), width = 4, height = 4)
ggplot(metadata, aes(Type_binned2, homeo_mean)) + 
    geom_boxplot(aes(fill=Type_binned2), outlier.size=0.5, width = 0.5) + 
    #geom_beeswarm(cex = 1, size = 1, alpha = 0.5)+
...
```{r}
pdf(paste0(out_dir, "loeffler_box_infl_final.pdf"), width = 4, height = 4)
ggplot(metadata, aes(Type_binned2, ifn_mean)) + 
    geom_boxplot(aes(fill=Type_binned2), outlier.size=0.5, width = 0.5) + 
    #geom_beeswarm(cex = 1, size = 1, alpha = 0.5)+
    geom_quasirandom(method = "pseudorandom", size = 1, alpha = 0.3)+
    #stat_density_2d() +
...
## Clustering based on chemokine expression (Fig. 5A)
```{r}
...
```{r, fig.height=8, fig.width=8}
pdf(paste0(out_dir, "loeffler_Fig5A.pdf"), width = 10, height = 8)
combined_plot
dev.off()
```

```{r}
pdf(paste0(out_dir, "loeffler_Fig5A_expression.pdf"), width = 5, height = 3)
p1 + p2
dev.off()
```
...
pdf(paste0(out_dir, "loeffler_homeo_km.pdf"), width = 5, height = 4)
print(plot_surv)
dev.off()
```
...
dp <- ggplot(homeo_expr, aes(homeostatic, homeo_mean, color = res.cat)) + 
  ...
  theme_bw()+
  #scale_y_continuous(limits = c(-3, 0.5), breaks = c(-3,-2,-1,0))+
  #scale_x_discrete(labels=c(
  #  "Sulfosuccinimidyl Oleate" = "SO",
  #  "10,12-Tricosadiynoic acid" = "10,12-TA"))+
...
pdf(paste0(out_dir, "loeffler_homeo_expression_dot.pdf"), width = 2, height = 4)
print(dp)
dev.off()
```
...
# Save
saveRDS(cox_data, file = "/path/loeffler_coxdata.rds")
```
...
pdf(paste0(out_dir, "loeffler_homeo_cox.pdf"), width = 8, height = 3)
forest_model(cox_model)
dev.off()
```


## Survival: FL Grade 1/2/3A ~ homeostatic chemokines (Extended Data Fig. 10G)
```{r}
## Prepare data

# Filter FL samples
metadata_filtered <- metadata[metadata$Type_binned %in% c("FL 1/2/3a"),]

# Filter out samples with available survival data
metadata_filtered <- metadata_filtered[metadata_filtered$OS_status %in% c(0,1),]
...
```{r}
pdf(paste0(out_dir, "loeffler_homeo_fl_km.pdf"), width = 5, height = 4)
print(plot_surv)
dev.off()
```

```{r, fig.width=2, fig.height=4}
homeo_expr <- mutate(metadata_filtered, res.cat = ifelse(homeo_mean > 3.292861, "High", "Low"))
homeo_expr$res.cat <- factor(homeo_expr$res.cat, levels = c("High", "Low"))
...
dp <- ggplot(homeo_expr, aes(homeostatic, homeo_mean, color = res.cat)) + 
  ...
  #  "Sulfosuccinimidyl Oleate" = "SO",
  #  "10,12-Tricosadiynoic acid" = "10,12-TA"))+
  geom_hline(yintercept = 3.292861, linetype = 2)+
  #coord_flip()+
  ylab("Mean expression")+
  xlab("")+
  ggtitle("")+
...
```{r}
pdf(paste0(out_dir, "loeffler_homeo_expression_fl_dot.pdf"), width = 2, height = 4)
print(dp)
dev.off()
```

## Survival: FL Grade 3B ~ homeostatic chemokines (Extended Data Fig. 10H)
```{r}
## Prepare data

# Filter FL3B samples
metadata_filtered <- metadata[metadata$Type_binned %in% c("FL 3b"),]
...
pdf(paste0(out_dir, "loeffler_homeo_fl3b_km.pdf"), width = 5, height = 4)
print(plot_surv)
dev.off()
```

```{r, fig.width=2, fig.height=4}
homeo_expr <- mutate(metadata_filtered, res.cat = ifelse(homeo_mean > 2.542142, "High", "Low"))
homeo_expr$res.cat <- factor(homeo_expr$res.cat, levels = c("High", "Low"))
...
dp <- ggplot(homeo_expr, aes(homeostatic, homeo_mean, color = res.cat)) + 
  ...
  #  "Sulfosuccinimidyl Oleate" = "SO",
  #  "10,12-Tricosadiynoic acid" = "10,12-TA"))+
  geom_hline(yintercept = 2.542142, linetype = 2)+
  #coord_flip()+
  ylab("Mean expression")+
  xlab("")+
  ggtitle("")+
...
pdf(paste0(out_dir, "loeffler_homeo_expression_fl3b_dot.pdf"), width = 2, height = 4)
print(dp)
dev.off()
```
...
pdf(paste0(out_dir, "loeffler_csx_minmax_nonB_hm.pdf"), width = 18, height =8)
draw(heatmap, heatmap_legend_side = "bottom", 
    annotation_legend_side = "bottom")
dev.off()
```
...
pdf(paste0(out_dir, "loeffler_csx_box.pdf"), width = 10, height = 6)
...
# Plot
pdf(paste0(out_dir, "loeffler_csx_chemokines_corrplot_FDR.pdf"), width = 7, height = 7)
...
pdf(paste0(out_dir, "loeffler_frc_ccl21_corr_disease.pdf"), width = 5, height = 5)
...
```{r}
pdf(paste0(out_dir, "loeffler_frc_cxcl12_corr_disease.pdf"), width = 5, height = 5)
...
### Correlation of FDC fraction and CXCL13 expression (Fig. 2D)
```{r}
csx_mod$Type_binned2 <- factor(csx_mod$Type_binned2, levels = c("Tonsil", "FL 1/2/3A", "FL 3B", "DLBCL"))
pdf(paste0(out_dir, "loeffler_fdc_cxcl13_corr_disease.pdf"), width = 5, height = 5)
...
pdf(paste0(out_dir, "loeffler_fdc_km.pdf"), width = 5, height = 4)
print(plot_surv)
dev.off()
```
...
pdf(paste0(out_dir, "loeffler_fdc_dot.pdf"), width = 2, height = 4)
print(dp)
dev.off()
```

### Chemokine UMAP ~ FDC fraction (Fig. 5D)
```{r}
...
```{r}
pdf(paste0(out_dir, "loeffler_Fig5_fdc_umap.pdf"), width = 3, height = 3)
...
pdf(paste0(out_dir, "reddy_homeo_km.pdf"), width = 5, height = 4)
print(plot_surv)
dev.off()
```
...
pdf(paste0(out_dir, "reddy_homeo_expression_dot_rev.pdf"), width = 4, height = 1.5)
...
# Save 
saveRDS(cox_data, file = "/path/reddy_coxdata.rds")
```
...
```{r, fig.height=6, fig.width=10}
pdf(paste0(out_dir, "reddy_homeo_cox.pdf"), width = 8, height = 4)
forest_model(cox_model)
dev.off()
```
...
# Step 4: Run stratified pairwise log-rank tests
...
# Step 5: Combine and (optionally) adjust p-values
